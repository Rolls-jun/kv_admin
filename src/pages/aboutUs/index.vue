  ...
  <div>
    ...
    >
        ...
        <a-col :md="24" :sm="24" :xs="24" :span="24">
          ...
          >
            <imgUpload
              ...
              multiple
              :maxCount="1"
              :urls="editData.banner_url"
              @uploadImgChange="uploadImgChange($event, 'banner_url')"
            />
          </a-form-item>
        </a-col>
      </a-row>
      <template v-if="visible">
        <a-card
          v-for="(level, levelIndex) in editData.levelList"
          :key="levelIndex"
          type="inner"
          :title="`序列组${levelIndex + 1}`"
        >
          <a slot="extra" href="#">
            <a-button
              v-if="levelIndex > 0"
              ...
              <a-col :md="12" :sm="12" :xs="12" :span="12">
                <a-form-item label="二维码图">
                  <imgUpload
                    ...
                    :urls="level.code_url"
                    @uploadImgChange="
                      uploadLevelChange($event, levelIndex, 'code_url')
                    "
                  />
                </a-form-item>
              </a-col>
              <a-col :md="12" :sm="12" :xs="12" :span="12">
                <a-form-item label="序列图">
                  <imgUpload
                    multiple
                    :maxCount="1"
                    :urls="level.level_url"
                    @uploadImgChange="
                      uploadLevelChange($event, levelIndex, 'level_url')
                    "
                  />
                </a-form-item>
              </a-col>
    ...
    <a-card v-else :style="{ minHeight: `${minHeight}px` }" :bordered="false">
      ...
      <a-descriptions title="banner图" bordered style="margin-top: 20px">
        <a-descriptions-item
          v-if="detailData.banner_url.length > 0"
          label="banner图"
          span="3"
        >
      ...
      <a-descriptions
        v-for="(level, index) in detailData.levelList"
        :key="index"
        :title="`序列_${index + 1}`"
        bordered
      ...
      >
        <a-descriptions-item label="二维码图" span="2">
          <img
            ...
            v-for="(code, urlIndex) in level.code_url"
            :key="urlIndex"
            :src="code.url"
          />
        </a-descriptions-item>
        <a-descriptions-item label="序列图" span="2">
          <img
            style="width: 80px; height: 80px"
            v-for="(level, urlIndex) in level.level_url"
            :key="urlIndex"
            :src="level.url"
          />
        </a-descriptions-item>
...
import { webConfigAbout, getWebConfigAbout } from "../api";
const editData = {
  banner_url: [], //	banner
  levelList: [
    {
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      webConfigAbout({ about_us: JSON.stringify(this.editData) }).then(
        (res) => {
